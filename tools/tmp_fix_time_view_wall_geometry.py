...
resolve_replacement = r'''  /// Resolve only the wall-clock hours that need extra visual room.
  ///
  ...
  static List<double> _resolveHourHeightsPx({
    ...
    for (var i = 0; i < slots.length; i++) {
      ...
      final requiredHourHeight =
          (requiredSpanPx / wallSpanMinutes * 60.0)
              .clamp(baseHourHeightPx, kPlanTimeMaxHourHeightPx)
              .toDouble();

      final firstHour =
          (visibleStart / 60.0).floor().clamp(0, hourCount - 1);
      final lastMinute = math.max(visibleStart, visibleEnd - 0.001);
      final lastHour =
          (lastMinute / 60.0).floor().clamp(0, hourCount - 1);
...
s = replace_between(
    s,
    '  /// Resolve each hour independently.',
    '\n  static ({\n    TimeViewYScale grid,',
    resolve_replacement,
)
...
assert_func = s.index('  static void assertPlanTimeViewLayoutDebug({')
first_loop = s.index('    for (var i = 0; i < layouts.length; i++) {', assert_func)
pair_loop = s.index('    for (var i = 0; i < layouts.length - 1; i++) {', first_loop)
first_replacement = r'''    for (var i = 0; i < layouts.length; i++) {
...
'''
s = s[:first_loop] + first_replacement + s[pair_loop:]

pair_loop = s.index('    for (var i = 0; i < layouts.length - 1; i++) {', assert_func)
hour_loop = s.index('    for (var h = 0; h < yScale.visibleHours.length; h++) {', pair_loop)
...
'''
s = s[:pair_loop] + pair_replacement + s[hour_loop:]
