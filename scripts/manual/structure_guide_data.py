...
FOLDERS: dict[str, dict[str, str]] = {
    ...
    "lib/core": {
        ...
        "inside": "`theme.dart`, `core/widgets/` (AppButton, plan cards), `core/services/` (desktop voice).",
    ...
    "lib/shared": {
        "what": "Cross-cutting shared code with multiple independent consumers — time, diagnostics, voice pipeline markers.",
        "why": "Avoid dumping single-feature helpers here; only multi-consumer ownership belongs under shared.",
        "inside": "`time/` — wall-clock and timezone; `diagnostics/` — runtime logs + kill switches; `voice/diagnostics/` — desktop voice pipeline markers.",
        "affects": "Header clock, Brain projection, Plans Time View, settings timezone labels, kill switches, desktop voice tracing.",
        "when": "Moving multi-consumer time/diagnostics, wrong today, DST labels, kill-switch toggles, voice pipeline tracing.",
        "delete": "No — Brain and UI need shared time and diagnostics.",
        "related": "`lib/shared/time/`, `lib/shared/diagnostics/`, `lib/shared/voice/diagnostics/`, `docs/APP_STRUCTURE.md`.",
    },
    ...
    "lib/shared/voice": {
        "what": "Shared voice helpers used by desktop STT/voice services and Brain voice parsing.",
        "why": "Voice pipeline markers must stay free of feature UI and Brain I/O while serving multiple consumers.",
        "inside": "`diagnostics/` — desktop voice log + pipeline step helpers.",
        "affects": "Desktop voice tracing and STT pipeline markers.",
        "when": "Desktop voice pipeline steps missing or quiet in debug/profile.",
        "delete": "No — desktop voice services import these markers.",
        "related": "`lib/shared/voice/diagnostics/`, `lib/core/services/desktop_voice_*.dart`.",
    },
    "lib/shared/voice/diagnostics": {
        "what": "Desktop voice pipeline markers — concise log + step helpers.",
        ...
        "affects": "Developer diagnostics for desktop voice only.",
        "when": "Tracing desktop voice capture, STT, overlay, or submit steps.",
        "delete": "No — used by debug/profile desktop voice builds.",
        "related": "`lib/core/services/`, `lib/shared/diagnostics/`.",
    },
    "lib/features/planning/diagnostics": {
        "what": "Planning marker-only duplicate / stream log used by Brain plan helpers.",
        "why": "Plan duplicate detection is Planning-owned but consumed only from Brain plan code — narrow Brain→features exception.",
        ...
        "delete": "No — Brain plan helpers import this marker module.",
        "related": "`lib/data/plans/`, `lib/shared/diagnostics/performance/runtime_flags.dart`.",
    },
    "lib/shared/time": {
    ...
    "lib/features/settings": {
        "what": "Settings-owned timezone helpers and labels (not design-system widgets).",
        "why": "Profile timezone option lists and legacy offset helpers belong to settings, not core.",
        "inside": "`timezone_settings.dart` (re-exports shared catalog/options for settings UI).",
        "affects": "Profile timezone selection labels and Planning category-default TZ search imports.",
        "when": "Timezone option list, offset label helpers.",
        "delete": "No — settings timezone helpers break.",
        "related": "`lib/shared/time/`, `lib/features/profile/profile_view.dart`.",
    },
    "lib/data/cache": {
    ...
    "installer/windows/stt_helper_src": {
        ...
        "related": "`installer/windows/stt_helper_build/`, `lib/core/services/desktop_stt_helper_service.dart`.",
    },
...
FOLDER_INFERENCE: list[tuple[str, dict[str, str]]] = [
    ...
    (
        "lib/features/planning/diagnostics",
        {
            "what": "Planning marker-only plan duplicate / stream log.",
            "why": "Owned by Planning; imported only by Brain plan helpers (narrow Brain→features exception).",
            "inside": "`plan_duplicate_log.dart`.",
            "affects": "Plan duplicate diagnostic markers only.",
            "when": "Investigating duplicate plan rows.",
            "delete": "No — Brain plan helpers import this module.",
            "related": "`lib/data/plans/`.",
        },
    ),
    (
        "lib/core/services",
