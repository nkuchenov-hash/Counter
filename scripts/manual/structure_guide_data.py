...
FOLDERS: dict[str, dict[str, str]] = {
    ...
    "lib/core": {
        "what": "Foundation layer — theme colors, shared widgets, desktop voice services, diagnostics.",
        "why": "Feature screens must not duplicate buttons, date headers, or voice plumbing; this is the design-system and utility base.",
        "inside": "`theme.dart`, `core/widgets/` (AppButton, plan cards), `core/services/` (desktop voice), `core/diagnostics/`.",
        "affects": "Look and feel on every tab; desktop voice/tray; header chrome widgets; perf debug flags.",
        "when": "Button/card migration, desktop voice broken, theme token change.",
        "delete": "No — features import foundation code everywhere.",
        "related": "`docs/DESIGN_SYSTEM.md`, `lib/features/`, `lib/shared/time/`, `lib/data/models.dart` (types only).",
    },
    "lib/features": {
    ...
    "lib/shared": {
        "what": "Cross-cutting shared code with multiple independent consumers — currently time only.",
        "why": "Avoid dumping single-feature helpers here; only multi-consumer ownership belongs under shared.",
        "inside": "`time/` — wall-clock, timezone catalog, app clock hooks, plan window/labels.",
        "affects": "Header clock, Brain projection, Plans Time View, settings timezone labels.",
        "when": "Moving multi-consumer time, wrong today, DST labels.",
        "delete": "No — Brain and UI need shared time.",
        "related": "`lib/shared/time/`, `docs/APP_STRUCTURE.md`.",
    },
    "lib/shared/time": {
