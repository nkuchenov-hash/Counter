...
FOLDERS: dict[str, dict[str, str]] = {
    ...
    "lib/data/categories": {
        ...
        "related": "`lib/data/category_service.dart`, `lib/features/settings/categories/`, `lib/shared/categories/`.",
    },
    "lib/data/records": {
    ...
    "lib/features/settings/categories": {
        "what": "Category manager UI — band grid, editor/appearance sheets, create dialog, browse panel.",
        "why": "Users organize work/personal/client categories from More → Categories (Settings ownership).",
        "inside": "List view, row widget, editor/appearance sheets, helpers, create dialog, recursive browse panel.",
        "affects": "More → Categories route only; shared pickers live under `lib/shared/categories/`.",
        "when": "Create category, reorder, archive, edit keywords/appearance.",
        "delete": "No — category management UI gone.",
        "related": "`lib/data/categories/`, `lib/shared/categories/`.",
    },
    "lib/features/dev": {
    ...
    "lib/shared": {
        ...
        "inside": "`time/` — wall-clock and timezone; `diagnostics/` — runtime logs + kill switches; `voice/` — one Voice system; `categories/` — reusable presentation/tree/picker/visibility.",
        "affects": "Header clock, Brain projection, Plans Time View, settings timezone labels, kill switches, Voice activation across platforms, shared category pickers.",
        "when": "Moving multi-consumer time/diagnostics/voice/categories, wrong today, DST labels, kill-switch toggles, Voice pipeline tracing, category picker ownership.",
        "delete": "No — Brain and UI need shared time, diagnostics, Voice, and Categories.",
        "related": "`lib/shared/time/`, `lib/shared/diagnostics/`, `lib/shared/voice/`, `lib/shared/categories/`, `docs/APP_STRUCTURE.md`.",
    },
    "lib/shared/categories": {
        "what": "Reusable category presentation, tree, picker, and local visibility preferences.",
        "why": "Multiple independent UI features share category pickers without owning Brain mutation or Settings manager chrome.",
        "inside": "`presentation/`, `tree/`, `picker/`, `visibility/`.",
        "affects": "Category tree pickers, form fields, create-from-picker dialog, hidden-category prefs — not PocketBase CRUD.",
        "when": "Category picker/create ownership moves, shared tree chrome, visibility prefs key.",
        "delete": "No — Notes/Planning/Timeline/Lists category pickers break.",
        "related": "`lib/data/categories/`, `lib/features/settings/categories/`, `lib/features/lists/`.",
    },
    "lib/shared/categories/presentation": {
        "what": "Injected category presentation lookup for plan cards (color/icon/breadcrumb).",
        "why": "Plan cards must resolve category chrome without importing Brain.",
        "inside": "`plan_category_lookup.dart` (`PlanCategoryLookup`, `PlanCategoryPresentation`).",
        "affects": "Plan card category color/icon/breadcrumb display.",
        "when": "Category presentation wiring or plan card category chrome.",
        "delete": "No — plan cards lose category presentation.",
        "related": "`lib/main.dart` `_wirePlanCategoryLookup()`, `lib/core/widgets/plan_time_task_card/`.",
    },
    "lib/shared/categories/tree": {
        "what": "Pure category-tree filter/opacity helpers and reusable tree body UI.",
        "why": "Picker and settings browse share one tree body without Brain imports.",
        "inside": "`category_tree_filter.dart`, `category_tree_body.dart`.",
        "affects": "Shared tree rendering and search filtering.",
        "when": "Tree opacity, expand path, picker search filter.",
        "delete": "No — shared category tree UI breaks.",
        "related": "`lib/shared/categories/picker/`, `lib/features/settings/categories/`.",
    },
    "lib/shared/categories/picker": {
        "what": "Category tree picker sheet, form field, create dialog, and narrow Brain injection contracts.",
        "why": "Picker create/select is multi-feature UI; mutation stays injected from composition root.",
        "inside": "`category_picker_contracts.dart`, models, create dialog, `category_tree_picker.dart`.",
        "affects": "Notes/Planning/Timeline/Wear category pickers and create-from-picker.",
        "when": "Picker create root/child semantics, sheet chrome, form field.",
        "delete": "No — category pickers break.",
        "related": "`lib/main.dart` `_wireCategorySharedContracts()`, `lib/features/lists/category_filter_tree_field.dart`.",
    },
    "lib/shared/categories/visibility": {
        "what": "Local-only hidden category preference storage (SharedPreferences).",
        "why": "Hide unused categories client-side without schema changes; reusable across Lists/shell/manager.",
        "inside": "`category_visibility_prefs.dart` (key `hidden_category_ids_json`).",
        "affects": "Hidden category filtering in pickers, Lists chips, Categories manager.",
        "when": "Hidden prefs, ancestor hide, edit-mode show-all.",
        "delete": "No — local hide behavior breaks.",
        "related": "`CategoryTreeSource.pathFromRoot` injection.",
    },
