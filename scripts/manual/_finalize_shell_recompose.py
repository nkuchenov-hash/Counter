...
entry = '''## 2026-08-18 — Shell lifecycle/chrome recomposition [engineering]\n\n- Reduced `lib/app/shell/app_shell.dart` to the dashboard composition root and shared state contract.\n- Extracted dashboard initialization/disposal wiring to `shell_lifecycle.dart` and responsive scaffold/chrome to `shell_chrome.dart`; shell navigation behavior and data behavior are unchanged.\n- Moved Paths selected-index ownership fully into `shell_more_menu.dart` instead of a dashboard-state override.\n\n'''
if not text.startswith('## 2026-08-18 — Shell lifecycle/chrome recomposition [engineering]'):
    changelog.write_text(entry + text, encoding='utf-8')

# Detailed guide: add folder-file entries without invoking the known noisy global generator.
detailed = ROOT / 'docs/APP_STRUCTURE_DETAILED.md'
text = detailed.read_text(encoding='utf-8')
for filename, purpose_en, purpose_ru in [
    (
