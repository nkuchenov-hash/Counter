...
LIB_FILE_GUIDES: dict[str, dict[str, str]] = {
    ...
    "lib/shared/categories/picker/create_category_from_picker.dart": {
        ...
        "why": "Edit sheets and shared tree must create a child under a known parent without inferring from search UI.",
        ...
        "why_ru": "Edit sheets и shared tree создают потомка под известным родителем без угадывания из поиска.",
        "contains_ru": "`CategoryPickerCreateTarget`, хелпер submit, виджет диалога создания.",
        "responsibilities_ru": "Собирает имя и родителя; вызывает создание через injected Brain; возвращает новый local id категории.",
    },
    "lib/shared/categories/presentation/plan_category_lookup.dart": {
        "what": "`PlanCategoryLookup` / `PlanCategoryPresentation` — injected category color/icon/breadcrumb for plan cards.",
        "why": "Plan cards must resolve category chrome without importing Brain.",
        "contains": "Presentation DTO + static resolve hook wired from `main.dart`.",
        "responsibilities": "Hold injected lookup; no PocketBase I/O.",
        "what_ru": "`PlanCategoryLookup` / `PlanCategoryPresentation` — injected цвет/иконка/breadcrumb категории для plan cards.",
        "why_ru": "Plan cards получают chrome категории без импорта Brain.",
        "contains_ru": "DTO презентации + static resolve hook из `main.dart`.",
        "responsibilities_ru": "Хранит injected lookup; без PocketBase I/O.",
    },
    "lib/shared/categories/visibility/category_visibility_prefs.dart": {
        "what": "`CategoryVisibilityPrefs` — local hidden-category ids (`hidden_category_ids_json`).",
        "why": "Hide unused categories client-side without schema changes.",
        "contains": "SharedPreferences load/persist, ancestor hide, filterPairs.",
        "responsibilities": "Local-only quarantine; path ancestry via `CategoryTreeSource`.",
        "what_ru": "`CategoryVisibilityPrefs` — локальные скрытые id категорий (`hidden_category_ids_json`).",
        "why_ru": "Скрывает неиспользуемые категории на клиенте без смены схемы.",
        "contains_ru": "SharedPreferences load/persist, hide предков, filterPairs.",
        "responsibilities_ru": "Только локальный quarantine; ancestry через `CategoryTreeSource`.",
    },
    "lib/features/shared/edit_sheet/category_edit_draft.dart": {
        "what": "Plan/record category draft resolve + plan PATCH category relation gate.",
        "why": "Explicit picker/create id must own edit Save without stale pair fallback.",
        "contains": "`resolvePlanningEditDraftCategoryId`, `planPatchShouldIncludeCategoryRelation`, edit field resolvers.",
        "responsibilities": "Pure draft/PATCH decisions for edit sheets.",
        "what_ru": "Resolve черновика category для plan/record + gate relation в plan PATCH.",
        "why_ru": "Явный id из picker/create владеет Save без stale pair fallback.",
        "contains_ru": "`resolvePlanningEditDraftCategoryId`, `planPatchShouldIncludeCategoryRelation`, resolvers edit field.",
        "responsibilities_ru": "Чистые решения draft/PATCH для edit sheets.",
    },
