...
// Cloud-owned sleep sync. Google Health API is the supported 2026 cloud API.
// The legacy provider value "google_fit" is retained only for collection compatibility.
...
function __healthFindExisting(app, userId, session) {
    ...
    // One-time compatibility with the abandoned direct Google Fit client.
    try {
        return app.findFirstRecordByFilter(
            "records",
...
function __healthConnectHandler(e) {
    var cfg = __healthGoogleConfig();
...
function __healthCallbackHandler(e) {
    ...
    try {
        ...
        // First successful authorization performs the historical backfill immediately.
...
// Legacy route name retained so existing Life OS clients do not need a coordinated release.
routerAdd("POST", "/api/sleep-sync/google-fit/connect", __healthConnectHandler, $apis.requireAuth("profiles"));
routerAdd("GET", "/api/sleep-sync/google-fit/callback", __healthCallbackHandler);
...
cronAdd("lifeos_google_health_sleep_sync", "*/15 * * * *", function() {
    ...
    for (var i = 0; i < connections.length; i++) {
        try {
            ...
            // Finish initial backfill at the next cron tick even if OAuth callback sync failed/timed out.
