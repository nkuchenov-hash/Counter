...
- `lib/data/` must not import `lib/features/` except the narrow marker-only path `lib/features/planning/diagnostics/` (plan duplicate log; consumers are Brain plan helpers only).
