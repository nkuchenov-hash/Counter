...
async function getJson(url) {
  const r = await fetch(url, { headers: { 'user-agent': 'IgropoiskAggregator/0.2' } });
...
function qualityGate(game) {
  const hasMetadata = Boolean(game.title && game.developers?.length && game.publishers?.length && game.genres?.length);
  const hasMedia = Boolean(game.headerImage && game.capsuleImage && game.screenshots?.length);
  const hasPopularity = Boolean(game.criticRating || game.recommendations >= 500 || game.peakConcurrent >= 100 || /100,000|200,000|500,000|1,000,000|2,000,000|5,000,000|10,000,000|20,000,000|50,000,000|100,000,000/.test(game.owners || ''));
  return hasMetadata && hasMedia && hasPopularity;
}
