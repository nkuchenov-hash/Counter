...
///
/// **Do not enable in release.** [rebuildMetricsTick] and [RebuildMetrics.instance] calls are no-ops
/// unless [kRebuildMetricsEnabled] is true. Used by shell/header for local bisect only.
/// Master switch — off in release; enable locally with `--dart-define=PERF_DIAG=true`.
const bool kRebuildMetricsEnabled = bool.fromEnvironment(
  'PERF_DIAG',
  defaultValue: false,
);
...
class RebuildMetrics {
  RebuildMetrics._();
  static final RebuildMetrics instance = RebuildMetrics._();
  ...
  void attachIfNeeded() {
    if (!kRebuildMetricsEnabled || _attached) return;
    _attached = true;
  ...
  void rebuild(String widgetName) {
    if (!kRebuildMetricsEnabled) return;
  ...
  T perfBlock<T>(String name, T Function() fn, {Map<String, Object?> meta = const {}}) {
    if (!kRebuildMetricsEnabled) return fn();
  ...
  void dateSwipeDrag({
    ...
    if (!kRebuildMetricsEnabled || _activeSection != section) return;
  ...
  void scheduleAutoSwipeSequence({
    ...
    if (!kRebuildMetricsEnabled || !kPerfAutoSwipeBenchmark || !shellTabActive) {
      return;
    }
...
void rebuildMetricsTick(String widgetName) {
  RebuildMetrics.instance.rebuild(widgetName);
}
