...
| **user_id** | String | **Owner** | **YES** | Owner UUID string (matches profile user_id). |
...
| **category_id** | String | **Data** | **YES** | Mirror of category business id (for filters / legacy). |
| **category_link**| Relation | **Expand** | **YES** | Relation to `categories`. **REQUIRED** for timeline: `expand: category_link`. |
...
2. **REST URL segment**: Use `/api/collections/records/records/{id}` for PATCH/DELETE. The **wrapper `id`** goes in the last segment.
3. **Sacred Singleton Law**: Before starting a new record, the Brain MUST stop open intervals for **projected today** (profile wall-clock day of `start_time`), then optionally merge **at most one** oldest **pre-today** open row (multi-day `running` handoff). Fire concurrent standard PATCH requests to change to `stopped`, then POST the new row.
4. **Clean Slate**: `date_key` and `is_manual` are strictly banned.
5. **PATCH 404 recovery**: If `PATCH .../records/{id}` returns 404 but the row still exists locally, the Brain re-queries PocketBase by `record_id` (UUID) using `?filter=(record_id='...')` to fetch the fresh system `id` and retries.
6. **Concurrent PATCH (preferred)**: PocketBase lacks native bulk arrays. The Brain must use `Future.wait()` (Dart) to fire concurrent single-row `PATCH` requests, or use a custom Go backend route for atomic bulk operations.
7. **Concurrent DELETE (preferred)**: Same pattern — fire concurrent `DELETE` requests to `.../records/{id}`.
8. **Timeline day bucket**: Rows are grouped into a calendar day using **profile wall-clock** (`start_time` UTC + profile offset), **not** the raw UTC `YYYY-MM-DD` prefix of `start_time`.
9. **Optimistic timeline UI**: The Brain merges **in-memory-only** overlays into the timeline before HTTP completes. Overlays are cleared when the server round-trip succeeds, **reverted** on failure, or on sign-out.
10. **Stats day scope**: Durations use **`timezone_offset`** (numeric) with `utcRangeForWallClockDate` for the **selected timeline day**. The aggregated-stats cache key includes **per-record** ids/times.
...
| **user_id** | String | **Owner** | **YES** | Owner UUID string. |
...
| **user_id** | String | **Owner** | **YES** | Owner UUID string. |
| **category_id** | String | **Data** | **YES** | Relation/Text mapping to the Categories collection. |
...
4. **Extensibility:**
   * Add new settings as new fields in the PocketBase collection dashboard. Existing API payloads won't break.
