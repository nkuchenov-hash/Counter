...
const String _plannerBaselineMigrationV7 =
    'lifeos_planner_baseline_migration_v7';
...
List<Map<String, dynamic>> _pathActionsFromStageRawV4(
...
ProjectPathAuditV4 auditExecutableProjectPathV4(
  ...
  for (var si = 0; si < root.checklist.length; si++) {
    ...
    if (stageTitle.isEmpty)
      structure.add('Этап ${si + 1}: нет названия результата.');
    if (doneWhen.isEmpty) {
      structure.add(
        'Этап ${si + 1}: не указано проверяемое условие завершения.',
      );
    }
    text
      ..write(' ')
    ...
    for (var ai = 0; ai < actions.length; ai++) {
      ...
      if (actionText.isEmpty) {
        structure.add(
          'Этап ${si + 1}, действие ${ai + 1}: нет физического действия.',
        );
      }
      if (result.isEmpty) {
        structure.add(
          'Этап ${si + 1}, действие ${ai + 1}: не указан результат.',
        );
      }
      if (minutes < 1 || minutes > 30) {
        structure.add(
          'Этап ${si + 1}, действие ${ai + 1}: время должно быть 1–30 минут.',
        );
      }
  ...
  if (profile == null) {
    return ProjectPathAuditV4(
      ...
      missingTracks: const [],
      missingTopics: const [
        'Для этого проекта ещё не задана предметная проверка слепых зон.',
      ],
    );
  }
  ...
  return ProjectPathAuditV4(
    audited:
        structure.isEmpty && missingTracks.isEmpty && missingTopics.isEmpty,
    structureProblems: structure,
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '01-01',
          'Открыть кабинет FLOW и выписать названия всех доступных модулей курса по порядку',
          'Список модулей в фактическом порядке курса',
          20,
          'learning',
        ),
        _actionV4(
          p,
          '01-02',
          'Отметить у каждого модуля: пройден / начат / не начат',
          'Точная текущая позиция в курсе',
          10,
          'learning',
        ),
        _actionV4(
          p,
          '01-03',
          'Выписать из текущего и следующего модуля все задания, которые требуют что-то создать или опубликовать',
          'Список ближайших практических заданий курса',
          20,
          'course_assignment',
        ),
        _actionV4(
          p,
          '01-04',
          'Отдельно выписать все площадки и каналы, которые курс требует создать по ходу обучения',
          'Список каналов, которые появляются из заданий курса, а не из догадок',
          15,
          'channel',
        ),
        _actionV4(
          p,
          '01-05',
          'Создать короткий шаблон заметки для каждого урока: идея → действие → результат → где ещё применить',
          'Один повторяемый шаблон конспекта урока',
          15,
          'playbook',
        ),
        _actionV4(
          p,
          '01-06',
          'Выбрать следующий непройденный урок строго по порядку курса',
          'Однозначно выбран следующий урок',
          5,
          'learning',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '02-01',
          'Пройти следующие 25 минут текущего непройденного урока FLOW',
          'Следующий фрагмент урока реально просмотрен/изучен',
          25,
          'learning',
        ),
        _actionV4(
          p,
          '02-02',
          'Записать после текущего фрагмента 3–5 конкретных приёмов или правил, которые можно повторить',
          'Короткий конспект без общих пересказов',
          10,
          'playbook',
        ),
        _actionV4(
          p,
          '02-03',
          'Определить ближайшее физическое действие из задания текущего урока',
          'Одно конкретное следующее действие задания ≤30 минут',
          10,
          'course_assignment',
        ),
        _actionV4(
          p,
          '02-04',
          'Выполнить первые 30 минут ближайшего задания курса',
          'Материальный прогресс по заданию: созданный аккаунт, текст, настройка, публикация или другой артефакт',
          30,
          'course_assignment',
        ),
        _actionV4(
          p,
          '02-05',
          'Сохранить ссылку, скриншот или другой результат выполненного задания и отметить его в конспекте урока',
          'Проверяемое доказательство выполнения задания',
          10,
          'validation',
        ),
        _actionV4(
          p,
          '02-06',
          'После завершения урока обновить в Path следующий урок и ближайшее задание из фактического курса',
          'Path продолжает реальную программу курса, а не шаблонные задачи',
          10,
          'learning',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '03-01',
          'Взять первый ещё не созданный канал из списка заданий курса и проверить требования к регистрации',
          'Понятно, какие данные нужны для регистрации конкретного канала',
          15,
          'channel',
        ),
        _actionV4(
          p,
          '03-02',
          'Создать аккаунт/канал на первой требуемой курсом площадке',
          'Реально существующий канал',
          20,
          'channel',
        ),
        _actionV4(
          p,
          '03-03',
          'Заполнить минимальный профиль канала: имя, описание, ссылка и аватар/обложка, если они требуются',
          'Канал не выглядит пустой технической заготовкой',
          25,
          'channel',
        ),
        _actionV4(
          p,
          '03-04',
          'Выполнить первое контентное задание курса для этого канала',
          'Готовый материал строго по текущему заданию',
          30,
          'content',
        ),
        _actionV4(
          p,
          '03-05',
          'Опубликовать материал и сохранить URL/идентификатор публикации',
          'Первая реальная публикация в собственном канале',
          15,
          'channel',
        ),
        _actionV4(
          p,
          '03-06',
          'Повторить цикл для следующего канала только когда курс действительно требует его создать',
          'Каналы появляются вслед за программой курса, без лишнего распыления',
          10,
          'course_assignment',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '04-01',
          'Подготовить следующий материал или настройку, которую прямо требует текущий урок',
          'Готовый результат текущего задания',
          30,
          'content',
        ),
        _actionV4(
          p,
          '04-02',
          'Опубликовать/включить результат задания на соответствующем живом канале',
          'Задание находится в реальной среде, а не только в черновике',
          15,
          'channel',
        ),
        _actionV4(
          p,
          '04-03',
          'Записать доступный фактический результат задания: просмотры, переходы, подписки, ответы или другой показатель курса',
          'Один результат без оценки «кажется, сработало»',
          15,
          'validation',
        ),
        _actionV4(
          p,
          '04-04',
          'Записать одним предложением, что из этого задания стоит повторить, изменить или не использовать',
          'Одно практическое правило по результату теста',
          10,
          'playbook',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '05-01',
          'Выбрать один законченный модуль и выписать из него только применимые техники продвижения',
          'Список практических техник одного модуля',
          20,
          'playbook',
        ),
        _actionV4(
          p,
          '05-02',
          'Для каждой техники записать, какую проблему она решает и на каком типе проекта применима',
          'У каждой техники есть контекст применения',
          20,
          'playbook',
        ),
        _actionV4(
          p,
          '05-03',
          'Добавить к одной технике реальный пример из собственного канала FLOW',
          'Техника привязана к фактическому выполнению, а не только теории',
          15,
          'playbook',
        ),
        _actionV4(
          p,
          '05-04',
          'Отметить один приём, который не дал ожидаемого результата, и условие, при котором его не стоит повторять',
          'Playbook хранит не только успехи, но и ограничения',
          15,
          'validation',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '06-01',
          'Выбрать один освоенный приём FLOW и один активный проект, где он решает реальную текущую проблему',
          'Пара «приём → проект» с понятной причиной',
          15,
          'transfer',
        ),
        _actionV4(
          p,
          '06-02',
          'Разложить применение выбранного приёма в этом проекте на первое действие ≤30 минут',
          'Конкретный перенос навыка в проект',
          15,
          'transfer',
        ),
        _actionV4(
          p,
          '06-03',
          'Выполнить первое действие применения приёма в выбранном проекте',
          'Реальный артефакт/изменение в проекте',
          30,
          'transfer',
        ),
        _actionV4(
          p,
          '06-04',
          'После теста записать результат и решить, переносить ли этот приём в KADR, GOLOS, Игропоиск, IW17, RCC или другой проект',
          'Решение о повторном использовании техники на основании результата',
          15,
          'validation',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          '07-01',
          'Проверить в фактической программе курса, является ли affiliate/реферальная монетизация обязательным практическим заданием',
          'Явное решение: требуется курсом / не требуется',
          10,
          'course_assignment',
        ),
        _actionV4(
          p,
          '07-02',
          'Если требуется, выписать точную партнёрскую программу и условия регистрации из урока',
          'Конкретная программа и условия вместо абстрактного affiliate',
          15,
          'course_assignment',
        ),
        _actionV4(
          p,
          '07-03',
          'Если требуется, пройти доступную часть регистрации в партнёрской программе',
          'Фактический статус регистрации и следующий шаг',
          30,
          'course_assignment',
        ),
        _actionV4(
          p,
          '07-04',
          'Если требуется, проверить disclosure и способ получения выплат до публикации реферальных материалов',
          'Проверены обязательные правила и payout route',
          20,
          'validation',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'offer-01',
          'Выписать все функции текущей рабочей версии GOLOS без будущих идей',
          'Фактический список функций',
          20,
          'product',
        ),
        _actionV4(
          p,
          'offer-02',
          'Отметить функции, которые должны остаться бесплатными, чтобы бесплатная версия решала основную задачу диктовки',
        ...
        _actionV4(
          p,
          'offer-03',
          'Выписать три причины, за которые пользователь мог бы платить: экономия времени, качество или дополнительный сценарий',
        ...
        _actionV4(
          p,
          'offer-04',
          'Выбрать первую платную комплектацию максимум из трёх преимуществ',
          'Одна версия Pro для проверки',
          15,
          'offer',
        ),
        _actionV4(
          p,
          'offer-05',
          'Выбрать первую модель оплаты и стартовую цену для теста',
          'Конкретная модель оплаты и цена',
          15,
          'payments',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'website-01',
          'Выбрать домен GOLOS и проверить доступность',
          'Выбранный доступный домен',
          20,
          'website',
        ),
        _actionV4(
        ...
        _actionV4(
          p,
          'website-03',
          'Выбрать технологию и размещение сайта без лишней инфраструктуры',
          'Один способ публикации сайта',
          20,
          'website',
        ),
        _actionV4(
          p,
          'website-04',
          'Опубликовать минимальную главную страницу с рабочей ссылкой скачивания',
          'Публичная страница GOLOS',
          30,
          'website',
        ),
        _actionV4(
          p,
          'website-05',
          'Добавить на сайт блок Free/Pro и место для checkout',
          'Пользователь понимает разницу бесплатной и платной версий',
          25,
          'website',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'license-01',
          'Создать товар и тестовую цену в выбранной платёжной системе',
          'Тестовый товар GOLOS существует у продавца',
          20,
          'payments',
        ),
        _actionV4(
          p,
          'license-02',
          'Записать техническую связь «успешная оплата → платный доступ в GOLOS» и разбить её на действия ≤30 минут',
          'Исполнимый подплан лицензирования',
          30,
          'licensing',
        ),
        _actionV4(
          p,
          'license-03',
          'Проверить восстановление платного доступа после переустановки/нового устройства',
          'Проверенный restore flow или конкретный blocker',
          20,
          'licensing',
        ),
        _actionV4(
          p,
          'license-04',
          'Выбрать один адрес/форму поддержки и добавить его в приложение и сайт',
          'Один рабочий support entry point',
          15,
          'support',
        ),
        _actionV4(
          p,
          'license-05',
          'Записать порядок выпуска новой версии: сборка → проверка → подпись → публикация → заметка',
          'Повторяемый release process',
          20,
          'operations',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'website-01',
          'Составить пять вариантов домена и проверить доступность',
        ...
        _actionV4(
          p,
          'website-02',
          'Выбрать домен и место регистрации',
          'Один выбранный домен и регистратор',
          15,
          'website',
        ),
        _actionV4(
          p,
          'website-03',
          'Выбрать технологию и хостинг первого сайта',
          'Один конкретный способ публикации',
          20,
          'website',
        ),
        _actionV4(
          p,
          'website-04',
          'Опубликовать пустой технический shell сайта на выбранном хостинге',
          'Реальный публичный URL сайта',
          30,
          'website',
        ),
        _actionV4(
          p,
          'website-05',
          'Подключить домен и проверить HTTPS',
          'Сайт открывается по основному домену без предупреждений',
          20,
          'website',
        ),
        _actionV4(
          p,
          'website-06',
          'Записать и проверить один способ резервного копирования/восстановления сайта',
          'Понятный backup/restore route',
          20,
          'operations',
        ),
        _actionV4(
          p,
          'website-07',
          'Подключить Search Console/Яндекс Вебмастер или соответствующие источники индексации',
          'Сайт добавлен минимум в один реальный search-data источник',
          20,
          'seo',
        ),
      ],
...
bool _projectPlanApprovedV5(PlanningTask root) =>
    _hasProjectPlanApprovalGateV5(root) &&
    root.checklist.first['isDone'] == true;

List<PlanningTask> _canonicalActivePathRootsV6(Iterable<PlanningTask> roots) {
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'baseline-01',
          'Открыть ZenMoney и разобрать первые 15 минут неразнесённых операций',
          'Меньше неразнесённых операций; исправленные категории сохранены',
          15,
          'operations',
        ),
        _actionV4(
          p,
          'baseline-02',
          'Выписать все источники дохода, по которым были реальные поступления за последние 3 полных месяца',
          'Список фактических источников дохода, а не потенциальных проектов',
          20,
          'finance',
        ),
        _actionV4(
          p,
          'baseline-03',
          'Выписать фактические поступления Price Reporter по месяцам, отдельно фиксированную часть и полученные бонусы',
          'Таблица фактических выплат Price Reporter по месяцам',
          25,
          'finance',
        ),
        _actionV4(
          p,
          'baseline-04',
          'Выписать фактические поступления Atozed и остальных источников по тем же месяцам',
          'Сопоставимая таблица остальных доходов',
          25,
          'finance',
        ),
        _actionV4(
          p,
          'baseline-05',
          'Выписать общие расходы за те же 3 месяца из ZenMoney без попытки оптимизировать их',
          'Три фактических месячных значения расходов',
          15,
          'finance',
        ),
        _actionV4(
          p,
          'baseline-06',
          'Посчитать средний фактический месячный доход и разрыв до 600 000 ₽',
          'Одно текущее число среднего дохода и одно число разрыва до цели',
          15,
          'validation',
        ),
      ],
    ...
    _stageV4(
      ...
      [
        _actionV4(
          p,
          'review-01',
          'Записать пятишаговый еженедельный порядок проверки ZenMoney',
          'Короткий денежный review, который можно превратить в повторяющийся план',
          15,
          'operations',
        ),
        _actionV4(
          p,
          'review-02',
          'Определить, какие итоговые цифры LIFE OS должен читать из ZenMoney или получать вручную',
          'Минимальный набор финансовых показателей LIFE OS без копирования бухгалтерии',
          20,
          'operations',
        ),
        _actionV4(
          p,
          'review-03',
          'Проверить процедуру на одном фактическом обновлении и записать, что пришлось считать вручную',
          'Список оставшихся ручных операций для будущей автоматизации',
          20,
          'validation',
        ),
      ],
...
Future<CategoryRule?> _ensureMoneyManagementCategoryV5() async {
  ...
  final db = DatabaseService.instance;
  final status = db.classifyCategoryDisplayNameInput(
    'Управление деньгами / ZenMoney',
  );
...
Future<void> _ensureMoneyManagementPathV5() async {
  ...
  await DatabaseService.instance.addPlanningTask(
    PlanningTask(
      id: 0,
      title:
          'Поддерживать актуальную картину личных денег и управлять движением к 600 000 ₽ в месяц на основании фактических данных ZenMoney и реальных поступлений по каждому источнику.',
...
Future<void> _ensureProjectPlanApprovalGatesV5() async {
  ...
  for (final root in roots) {
    ...
    if (!ok)
      throw StateError(
        'Could not add plan approval gate to category ${root.categoryId}',
      );
  }
}

bool _checklistHasPrefixV4(
...
Future<void> _replacePathV4({
  ...
  for (final task in tasks) {
    ...
    if (!ok)
      throw StateError('Could not preserve previous Path for ${category.name}');
  }
  ...
  if (!created)
    throw StateError('Could not create revised Path for ${category.name}');
}
...
Future<void> _detachPriceReporterMoneyWorkV5() async {
  ...
  for (final rawStage in root.checklist) {
    ...
    if (id == 'exec-pricereporter-stage-1') {
      stage['text'] =
          'Зафиксировать фактическую рабочую нагрузку и критичные виды работы';
...
List<_WeekActionCandidateV4> _currentActionsForRootV4(
  ...
  if (targetStage == null ||
      targetStage < 0 ||
      targetStage >= root.checklist.length) {
...
Future<void> _migrateLegacyApprovalDuplicatesV7() async {
  ...
  final canonicalRootByCategory = <int, String>{
    for (final root in roots)
      root.categoryId: (root.pocketRecordId ?? root.planRowIdForBackend).trim(),
  };
...
/// Compatibility-only cleanup for planner rows generated by the old Paths
...
Future<PathWeekPlanReportV4> planCurrentWeekFromPathsV4() async {
  ...
  await _removeSupersededWeekRoutinesV5();
  final existingApprovalCategoryIds = await _currentWeekApprovalCategoryIdsV7();
  ...
  for (final row in allPlans) {
    ...
    if (title.isNotEmpty && dt != null) {
      existingTitlesByDate
          .putIfAbsent(_dateKeyV4(dt), () => <String>{})
          .add(title);
    }
  }
  ...
  final monday = _mondayOfV4(today);
  final weekdays =
      <DateTime>[for (var i = 0; i < 5; i++) monday.add(Duration(days: i))]
          .where(
  ...
  while (activeProfiles.isNotEmpty && safety++ < 500) {
    ...
    final rootId =
        (candidate.root.pocketRecordId ?? candidate.root.planRowIdForBackend)
            .trim();
    final marker =
        '$_pathActionPlanMarkerV4$rootId|${candidate.stageId}|${candidate.actionId}';
    ...
    if (existingMarkers.contains(marker) ||
      ...
      remainingBudget[profile] = budget - candidate.minutes;
      if ((indexes[profile] ?? 0) < candidates.length &&
          (remainingBudget[profile] ?? 0) > 0) {
    ...
    for (var attempt = 0; attempt < weekdays.length; attempt++) {
      ...
      final earliest =
          dayCursor[dk] ?? DateTime(day.year, day.month, day.day, 9);
