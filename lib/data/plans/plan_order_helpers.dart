...
/// Planning optimistic reorder, first-drag baseline, debounced diff-only order sync.
...
extension PlanOrderSyncExtension on DatabaseService {
...
/// @visibleForTesting Plan reorder baseline / pending-list harness.
extension PlanOrderSyncTestBridge on DatabaseService {
  @visibleForTesting
  Map<String, int>? debugPlanReorderBaselineForTest() {
    final m = _planReorderBaselineByPlanId;
    if (m == null) return null;
    return Map<String, int>.from(m);
  }

  @visibleForTesting
  List<String>? debugPendingPlanOrderIdsForTest() {
    final pending = _pendingPlanOrderSyncList;
    if (pending == null) return null;
    return [for (final t in pending) t.planRowIdForBackend];
  }

  @visibleForTesting
  void debugResetPlanOrderSyncForTest() {
    _cancelPlanOrderDebounceTimer();
    _pendingPlanOrderSyncList = null;
    _planReorderBaselineByPlanId = null;
  }
