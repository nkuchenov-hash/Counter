...
/// Category-default schedules are chain anchors, not gap-fill anchors.
/// Existing plans inheriting the same source rule stay in one append-only chain.
DateTime computePlanRuleChainAppendStart({
  required DateTime anchorStartWall,
  required int sourceCategoryId,
  required List<PlanningTask> existingDayPlans,
  required int? Function(int categoryId) resolveRuleSourceCategoryId,
  required int Function(List<Tag> tags) resolveDurationMinutes,
}) {
  var appendStart = anchorStartWall;
  for (final plan in existingDayPlans) {
    final start = plan.startTime;
    if (start == null ||
        resolveRuleSourceCategoryId(plan.categoryId) != sourceCategoryId) {
      continue;
    }
    final explicitEnd = plan.endDateTime;
    final end = explicitEnd != null && !explicitEnd.isBefore(start)
        ? explicitEnd
        : start.add(Duration(minutes: resolveDurationMinutes(plan.tags)));
    if (end.isAfter(appendStart)) appendStart = end;
  }
  return appendStart;
}
  ...
  resolveAutoPlanSchedule({
    ...
    if (explicitStartWall != null) {
      ...
      if (h != null && m != null) {
        ...
        final anchorStart = _snapPlanWallDateTime(
          DateTime(wallDay.year, wallDay.month, wallDay.day, h, m),
        );
        final sourceCategoryId = catSchedule.sourceCategoryId;
        final appendStart = sourceCategoryId == null
            ? anchorStart
            : computePlanRuleChainAppendStart(
                anchorStartWall: anchorStart,
                sourceCategoryId: sourceCategoryId,
                existingDayPlans: existingDayPlans,
                resolveRuleSourceCategoryId: (existingCategoryId) =>
                    effectiveDefaultPlanScheduleForCategory(
                      existingCategoryId,
                    )?.sourceCategoryId,
                resolveDurationMinutes: resolvePlanDurationMinutesFromTags,
              );
        startWall = _snapPlanWallDateTime(appendStart);
