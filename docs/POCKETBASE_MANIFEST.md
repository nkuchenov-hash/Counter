...
| `parent_plan_id` | relation | Optional self-relation → `plans.id`. For materialized recurrence exceptions, points to the owning recurrence series row. |
| `recurrence_instance_date_key` | text | Optional `YYYY-MM-DD` original occurrence date for a materialized recurrence exception. Used with `parent_plan_id` for dedupe and series-split reparenting. |
