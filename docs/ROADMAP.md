...
**Status (2026-06-10): F2A shipped + Android visual regression fix.** Safe UI-only slice completed: compact tab/segment scale stabilized for Android, Plans play button moved into the leading action column, repeat icon added directly beside recurring plan titles, and main tabs use a readable dark compact date/time header. F2 is **not** fully complete.
...
- ◐ **F2A — Recurring plan icon (#13 partial):** Plans with non-empty `rrule` or virtual recurrence instance state show a repeat icon beside the card title. Existing repeat controls live under a `Repeat` edit-sheet tab; `Parallel plans` remains a separate accessible tab. Full edit-scope dialog/mutation remains open.
...
- ◐ **F2A — Header audit (#1 partial):** Main tabs use a compact dark `Material` + `GlobalAppHeader` date/time bar with matching Android status bar color. Secondary route `AppBar` normalization remains follow-up.
