...
**Status (2026-06-11): F2A and F2C shipped/accepted; F2B deferred.** Safe UI-only slice completed: compact tab/segment scale stabilized for Android, Plans play button moved into the leading action column, repeat icon added directly beside recurring plan titles, and main tabs use a readable dark compact date/time header. F2C category default plan time is shipped with the manually added PocketBase `categories.default_plan_time` field and searchable selector UI. Remaining pixel-perfect tab/header/tag polish is deferred to **V3 UX_CONTRACT / V7 Design Language**. **Recurring virtual-occurrence time/metadata edit** (materialize + `exception_dates`) is shipped; broader series edit-scope dialogs remain open. **Time mode** accepts **5-minute** snap/min duration (see `planning_day_start_prefs.dart`).
