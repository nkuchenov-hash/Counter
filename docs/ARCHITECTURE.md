...
- **PERFORMANCE_KILL_SWITCH_LAW (Iron Rule — P0V):** Performance, responsiveness, and stability are **sacred**. The app must **never** become slower, crashier, less responsive, or less immediately reactive because of a preload/cache/render/design experiment. **Worst failure ≠ imperfect UI; worst failure = damaged speed, stability, instant feedback, or basic usability.**
  ...
  **Hot path rule:** Swipe, startup, text entry, record create/start/stop, plan create/update, tab switch, Flutter `build()`, sort comparators, painters, layout callbacks, gesture-update callbacks, high-frequency listeners, and periodic visual timers are hot paths. No full-history scan, full-plan projection, repeated full-list transformation, cache/database traversal, network wait, logging storm, or heavy rebuild may run **synchronously** on these paths.

  **Reactive UI hot-path law (added after the 2026-08-25 Plans > Time View CPU incident):**
  - **Compute once, reuse many times.** Expensive derived values such as timezone projections must be calculated once per item per rebuild/input revision and reused. Calling an expensive projection from a sort comparator is forbidden because comparator execution multiplies cost to `O(N log N)` comparisons.
  - **No duplicate full passes without proof.** A rebuild must not sort/project/normalize the same collection and then repeat the same full pass later in the same render path unless the second pass is unavoidable, documented, and covered by a regression test/budget.
  - **Offscreen pages are operationally idle.** `PageView`, `IndexedStack`, keep-alive, and prefetched pages may preserve UI state, but invisible pages must not perform domain refreshes, expensive projections, global-stream reactions, periodic timers, or resource loading unless that background behavior is explicitly required. Heavy listeners/work must be gated on active/visible state.
  - **Invalidate the smallest visual scope.** Clock/now-line/progress updates should rebuild/repaint only the local visual owner. A periodic timer must not trigger a full-page rebuild merely to move a local line. Timers, tickers, and subscriptions must be cancelled in `dispose()`.
  - **Release diagnostics stay off hot paths.** Per-plan/per-row lifecycle diagnostics default OFF in release and must not become part of normal render cost.
  - **Performance bugs become executable contracts.** Every confirmed hot-path regression must add a regression test or measurable budget preventing the same shape from returning. Plans > Time View is guarded by `test/plans_time_performance_contract_test.dart`: no projection inside the sort comparator, no duplicate projection/sort across the main hour-grid rebuild, active-page-only background work, bounded timer cadence, and release diagnostics disabled.
  - **Compilation is not performance proof.** Changes touching Time View projection/sorting, Planning lifecycle/listeners, PageView day mounting, periodic timers, or performance flags must run the focused performance contract plus relevant Time View tests and the normal architecture guard before merge.

  **Code anchors:** `lib/shared/diagnostics/performance/runtime_flags.dart`, `lib/shared/diagnostics/performance/shell_flags.dart`, `test/plans_time_performance_contract_test.dart`. Experimental preload/render paths default **off** until proven stable on **web and Android**.
...
- **Records realtime:** After a valid session, the Brain subscribes to `records` (`subscribe('*', …)`) so **Web and mobile** share the same in-memory cache updates from server pushes; login flows must re-arm this subscription if init ran before auth. `PB_CONNECT` transport recovery is single-owned by the Brain: a real SSE gap triggers one coalesced authoritative catch-up because PocketBase does not replay events missed while disconnected. Visible state propagation itself is event-driven; no domain polling delay is allowed.
...
Browser-local and open-tab snapshots are presentation caches only. They may render immediately, but they are never proof of freshness: each popup open performs a canonical freshness sync, and the open popup applies realtime record events immediately without waiting for navigation or a periodic refresh interval.

Start/Stop commands remain one-shot requests through the existing LIFE OS record/Highlander semantics. Request IDs remain locally deduped and a temporary bridge/bootstrap path must not close before the resulting canonical running state is confirmed.

This boundary preserves category inference, singleton-running semantics, optimistic/offline behavior, timezone rules, and the performance/no-glitch contracts.
