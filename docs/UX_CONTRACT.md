...
- **Horizontal swipe inside Timeline, Planning, and other date-based sections is date navigation** (previous/next calendar day). It is **required** product behavior — do not remove it, disable it globally, or replace it with tap-only navigation.
- **This is not main-tab navigation.** The app shell bottom/side nav switches sections; horizontal swipe within a section changes the **selected date** for that section.
- **Implementation:** `TimelineSwipeWrapper` and `PlanningSwipeWrapper` in `timeline_view.dart` / `planning_view.dart` — infinite `PageView.builder` (`DateSwipePager` constants in `lib/core/date_swipe/`).
- **Parent shell** (`app_shell.dart`) shares `_selectedDate` across tabs but must **not steal** section date swipe. Off-tab wrappers use `shellTabActive: false` so hidden `PageView`s do not animate or refetch during another tab’s date swipe.
- **During Planning Time-mode card drag or resize only:** horizontal date paging is temporarily locked (`NeverScrollableScrollPhysics` on the day `PageView`); card interaction owns the gesture. After drag/resize ends, date swipe is restored.
- **Vertical scroll** remains vertical scroll (hour grid `ListView`, record list, etc.). Date swipe must not fight vertical scroll or card drag.
- **Performance:** Date settle (`onPageChanged`) may update shared date and fetch data; **no** PocketBase fetch, full timeline projection, or shell-wide rebuild storm on every swipe frame. Debug diagnostics: `DateSwipePerfMonitor` (`DATE_SWIPE_*` logs, debug-only).
- **Tap** opens edit/actions; **long press** starts card drag where supported.
- Gesture conflicts (date swipe vs scroll vs card drag) are **P0 bugs**.
